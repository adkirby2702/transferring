...
namespace ConsoleApp1
{
    public class Pathfinder
    {
        ...
        {
            if (File.Exists("c:\\users\\alexk\\source\\repos\\consolepro\\index.html"))
            {
                File.Delete("c:\\users\\alexk\\source\\repos\\consolepro\\index.html");
            }
            File.WriteAllText("c:\\users\\alexk\\source\\repos\\consolepro\\index.html", "<!DOCTYPE html> \n <html>\n <head> Images crawled over </head> \n<br/> <body><ul>") ;
            ...
            Pathfinder.visited = (webUrl,title) =>
            {
                ...
                using (FileStream indexFile = File.Open("c:\\users\\alexk\\source\\repos\\consolepro\\index.html", FileMode.Append))
                {
                    Byte[] info = new UTF8Encoding(true).GetBytes("<li><a href='C:\\Users\\alexk\\Desktop\\screenshotTest\\screenshot" + allCheckedPages.Count().ToString() + ".png'> Page "+ allCheckedPages.Count().ToString()+ " </a></li>\n");
        ...
        private static void PathfinderThread()
        {
            while (true)
            {
                ...
                if (queueToCheck.Count == 0 || allCheckedPages.Count()==6)
                {
                ...
                //if (doc.DocumentNode.Descendants("h2").ToString() != null)
                //{
                  //  title = doc.DocumentNode.Attributes["h2"].Value;
                //}
                
                    title = "not provided";
                
                if (!allCheckedPages.ContainsKey(webUrl))
                {
                ...
                foreach (var link in doc.DocumentNode.SelectNodes("//a[@href]"))
                {
                    ...
                    if (allCheckedPages.ContainsKey(newLink.ToUpper()+"/"))
                    {
                        continue;
                    }
